    ...
    for a = 1:5
        ...
        for b = 1:2
            ...
            for i = 1:tot_s
                ...
                pxls_forest = sum(pxl(:) == 1)/area_f;
                % nonforest1
                pxl = double(nonfor1_clouddata.cloud_data_nonfor1(:,:,i));
                pxls_nonfor1 = sum(pxl(:) == 1)/area_nf1;
                % nonforest2
                pxl = double(nonfor2_clouddata.cloud_data_nonfor2(:,:,i));
                pxls_nonfor2 = sum(pxl(:) == 1)/area_nf2;
                
                % Calculating date from timestep
                [year, month, day, hour] = Calculate_date_from_timestep_yridx(i, yr_idx);
